...
public class WorldGenerator implements IWorldGenerator
{
	...
	private void generateSurface(World world, Random random, int blockX, int blockZ) 
	{
		...
		for(i = 0; i < 12; i++)
		{
			...
			int Zcoord = blockZ + random.nextInt(16);
			(new WorldGenMinable(CocoCraftBlocks.Essence.blockID, 0, 4)).generate(world, random, Xcoord, Ycoord, Zcoord);
		}
		for(i = 0; i < 12; i++)
		{
			//Essence Water
		...
		for(i = 0; i < 12; i++)
		{
			...
			int Ycoord = random.nextInt(128);
			int Zcoord = blockZ + random.nextInt(16);
			(new WorldGenMinable(CocoCraftBlocks.Essence.blockID, 2, 4)).generate(world, random, Xcoord, Ycoord, Zcoord);
		}for(i = 0; i < 12; i++)
		{
			//Essence Earth
			int Xcoord = blockX + random.nextInt(16);
			int Ycoord = random.nextInt(128);
			int Zcoord = blockZ + random.nextInt(16);
			(new WorldGenMinable(CocoCraftBlocks.Essence.blockID, 3, 4)).generate(world, random, Xcoord, Ycoord, Zcoord);
		}
	}
	...
	private void generateNether(World world, Random random, int blockX, int blockZ) 
	{
		for(int i = 0; i < 32; i++) {
			...
			int Ycoord = random.nextInt(128);
			int Zcoord = blockZ + random.nextInt(16);
			(new WorldGenMinableNether(CocoCraftBlocks.Essence.blockID, 4, 4)).generate(world, random, Xcoord, Ycoord, Zcoord);
		}
	}
